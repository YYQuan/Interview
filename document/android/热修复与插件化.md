...
Tinker 对比 QZone  tinker是增量更新， 下载的是差分包，而QZone是只要修改过的类都要下载  。 tinker 效率比Qzone高些， 所以Tinker比Qzone更优秀些。

Qzone 是多下发了一个包和本身的dex是两个dex文件， 所以又有 activity 是否只引用了本dex的下的资源的问题的了。
如果原本的activity 只引用了本dex下的资源的话，那么就会打上一个标记。
导致该再无法引用到别的dex的资源。
这就会导致Qzone的解决方案失效，Qzone的解决方式是 通过字节码插桩来然每一个都引用到别的dex的类后，再进行打包。
为啥要用字节码插桩来解决呢？因为无法直接引用到另一个dex ，dex又不可以import进来， 所以无法绕过编译器， 所以通过字节码插桩，在编译完了之后，再来插入代码，避免编译报错。这样就避免了这个问题。
...
- 工作流程  差分包 合并成完整的dex ，然后插入到classLoader的dex列表的队列的头部。
...
![image-20210728102355569](https://i.loli.net/2021/07/28/XzFL7Vh1xIeATgD.png)



- 类的加载
  ![image-20210728114409252](https://i.loli.net/2021/07/28/B6HkwjOoDEMNTzY.png)
  在启动的时候就拿到宿主的classLoader，

  然后把插件的classLoader需要插入到宿主classLoader当中去。

  这样就能避免 在宿主中对序列化后的插件里类进行反序列化的时候，由于没有class信息导致的异常。

  

  VirsualAPK 中插件之间 是可以相互访问的。
  而另一个插件化框架DroidPlugin 插件之间是相互隔离的。

  是怎么做到这点的呢？

  DroidPlugin中 插件的classLoader是关联的是宿主的父classLoader，也就是对顶级的BootClassLoader
  而VirsualAPK 关联的是宿主apk。

  所以DroidPlugin 的插件之间是互不感知的，甚至和宿主都是互不感知的。
  
- 资源的加载

  资源的加载本身 直接用资源路径去加就行了。
  但是需要处理资源id重复的问题

  资源重复的问题，直接通过aapt去设置插件包的资源头序即可。
  默认是7f ,插件包 不要用7f开头，就能避免资源id重
...
- 如何支持四大组件
  通过hook把 未在Mainfest中注册的组件，替换成占坑的组件；
  要注意要把对应的classLoader传入 要不在intent传 序列化参数的时候 会有异常
  因为宿主的classLoader中不一定有插件的class信息， 这样反序列化的时候就有有问题。
  所以才有前面的插件的classLoader 必须插入到宿主的classLoader当中。

  DroidPlugin的方案就比VirtualApk要好一些。
  DroidPlugin的把插件的组件的intent直接整个序列化 传给占坑组件的。
  然后到插件中的时候再进行反序列化。
...
![image-20210728112342072](https://i.loli.net/2021/07/28/OaIP9z2q8tC4m5h.png)



### Shadow -非Hook的插件化实现

想virtualApk启动四大组件是通过 hook来改变 传给ams的组件信息 从而达到欺骗系统的目的。
而另一种 插件化的实现方案是 通过代理来完成插件组件的启动的。

何为代理？
![image-20210729105407450](https://i.loli.net/2021/07/29/1RtOJgeNBXcjaFq.png)

其实就是预先注册好了代理组件， 插件的组件在编译 都不直接编成组件了。
而是编成了代理所管理的成员。插件的组件的生命周期 由代理类来负责转发控制。



静态代理的插件化方案的典型就是shadow.
静态代理方案的好处就是 不那么依赖系统版本。
virtualApk在系统api中hook的方案， 很容易由于系统版本变动而需要兼容处理。
而代理的方式的话， 依赖的是系统本来就暴露出来的api，这部分api是很少发生变化的。



![image-20210729110401255](https://i.loli.net/2021/07/29/2unYDlJyeiaXZhS.png)



使用上 在startActivity的时候 就得填入 ProxyActivity了， 不能想VirtualApk一样填，插件的activity了。

ProxyActivity在编译的时候  通过字节码插入修改了其逻辑，使得能够支持代理到插件里的activity.



#### 插件与宿主的关系

代理的方式 怎么做到 插件能够访问宿主的类呢？
通过 控制  使用的classLoader来控制  能否访问得到宿主的类。



#### 字节码编辑技术

![image-20210729111715517](https://i.loli.net/2021/07/29/JXQl5iR83EuhPBq.png)



主要是通过groovy的transfer  来处理的。
处理的是class文件 。
用javassist  会更方便 ， 更直观一点。
...
- 所有模块在一个工程当中
  那插件化的意义不是很大
- 大佬建议用 gradle 4 之后推出的 compositeBuild的功能来 依赖外部的工程。
...
- 是public的  但是被隐藏了 也就是@hide标注的这些
- 是private

对于public但是被hide的这种，直接把系统的代码拷贝过来欺骗了服务器 就能直接用了。

![image-20210727174809154](https://i.loli.net/2021/07/27/QMsnl87VaYGyEBD.png)

对于private的这种就只能使用反射了， 没法绕过编译器。

#### Android P  

Android P 的特殊之处在于 给api设置的名单。


![image-20210727180013827](https://i.loli.net/2021/07/27/k4boXOsxL9DrSyi.png)

在黑名单当中的api ，用反射也会抛出异常了。
...
android P 对反射的native函数加了一层判断

![image-20210727180317924](https://i.loli.net/2021/07/27/bKa1NptSrw4O5Bx.png)

那关键就是让GetMemberAction 不返回kDeny，就能避免反射被阻断了。



那解决的方法就是去hook getMemberAction的 返回。
...
发现是 jvm的runtime来操作的。所以只要能改动到jvm的runtime那么就能控制这个函数的返回了。
jvm这个容易拿到，但是runtime是系统定义的。
不能直接拿到。 

![image-20210727181220483](https://i.loli.net/2021/07/27/WknO9yjYbm3x5NQ.png)

github上有个开源框架来解决这个问题。
tiann/FreeReflection



FreeReflection方案的基本原理就是在C++ 通过获取ivm的指针后字节对齐
然后修改到 runtime。注意是通过字节对齐 ，然后自已定义个结构体来操作到runtime 属性。

核心重点就是 利用c++的内存布局   定义出类型 系统隐藏掉的结构体， 然后来修改。虽然我不能直接用系统的定义， 但是利用字节对齐，然后就模拟出了系统定义的结构体了。

### 如何实现换肤的功能

tvlauncher里面用的hook了resource的各种get来实现的。
 先在下发的配置中 看有无配置，  然后再用自带的。
目前能还 text 和除了带状态的之外的drawable

![image-20210727182710888](https://i.loli.net/2021/07/27/fcLET5RbZAwB6le.png)



常规方案

![image-20210727182941470](https://i.loli.net/2021/07/27/rFbaCYVxJA6suX5.png)

tvlauncher类似 这个包装流。
明显这个资源加载多了些判断， 性能上是会有影响的。
面试的话 就说这个我是基本epic上来进行修改的。
主要就解决当绘图过多时， epic会有 native的java对象全局引用溢出的问题。
我增加了主动去删除释放全局引用。

![image-20210727182926121](https://i.loli.net/2021/07/27/Os2typZwb7NRvXc.png)

![image-20210727183033339](https://i.loli.net/2021/07/27/YqFoVpwXi2hvZNC.png)
...
动态映射方案：
先用 值找名称， 再通过另一个包 来名称找值

![image-20210727183751249](https://i.loli.net/2021/07/27/PebSXHMq1QfVDWz.png)



![image-20210727183900092](https://i.loli.net/2021/07/27/RQji7pStFvPCxhO.png)
...
#### AssetManager替换流的基本实现

![image-20210728101003184](https://i.loli.net/2021/07/28/m7LzeF1MZWNADKv.png)
