...
RouteProcessor的核心就是遍历全部被Route注解修饰的类，
然后分组储存。
然后把同组的路由节点用javapoet来生成一个管理类。
等待用到该group的时候 再把该组的全部节点 维护进来。

javapoet生成的管理类如下：

其实就是 建立了一个 路由路径和节点 的 键值对。
...
public class ARouter$$Group$$account implements IRouteGroup {
  ...
  public void loadInto(Map<String, RouteMeta> atlas) {
    ...
    atlas.put("/account/registration", RouteMeta.build(RouteType.ACTIVITY, RegistrationActivity.class, "/account/registration", "account", null, -1, -2147483648));
  }
}
```
...
编译时把autowire都收集起来 ，然后用javapoet写入java文件当中。
参数的自动的实际就是 路由是把对应参数传到 intent中，
然后再被注入方 手动触发注入。
其实本质就只是少写了些 从intent中读值  赋值的语句。
...
public class ARouter$$Interceptors$$app implements IInterceptorGroup {
  ...
  public void loadInto(Map<Integer, Class<? extends IInterceptor>> interceptors) {
    interceptors.put(9, BizInterceptor.class);
  }
}
```
...
每一个模块下 aroute都有root节点来统一管理分组。
root节点和 路由节点 ，拦截器和 跨模块通讯的管理类都是在固定包名下的，
arouter初始化的时候 就会去把这个固定包下的全部文件加载进来。

而组下的自动注入的管理类 不会在这个固定包下，也不会在arouter初始化的时候加载而是按需加载。

生产的java的管理其实都只是把节点信息加载进arouter内部的键值对当中去。
