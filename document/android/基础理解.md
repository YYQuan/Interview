...
```
重新 描述一下
binder是应用进程来启动的时候就申请了内核空间（默认是1M），
并通过内存映射技术 来把用户空间映射到内核空间当中，

客户端调用服务端的接口时 就由binder驱动开辟了一块新的内核空间来作为接受缓存， 并且把接受缓存 映射到服务端的内核空间中。
这样就完成了   接收缓存 -》 服务端内存空间 -》 服务端用户空间的映射。
binder驱动再等到服务端处理完了之后，再把数据从接受缓存中，拷贝到客户端的用户空间中，就完整了整个的调用
```
