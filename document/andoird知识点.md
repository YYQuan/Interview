...
给Project 插入一个startTask ，和endTask,
然后再这两个任务之间插入任务，维护依赖关系。

然后start的时候就执行startTask,

startTask执行完了之后，就去通知所有的其后置任务。
收到通知的任务，检测一下自己的前置任务是不是都执行完了,
如果前置任务都执行完了的话，那么就可以开启执行。
